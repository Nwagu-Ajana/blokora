...
namespace Blokora.Gameplay
{
    ...
    public sealed class SoloGameSession
    {
        ...
        public bool TryPlace(int trayIndex, int x, int y, out ClearResult clear)
        {
            ...
            clear = Board.Place(piece, x, y); PiecesPlaced++; LinesCleared += clear.Lines;
            Combo = clear.Lines > 0 ? Combo + 1 : 0; Score += ScoreRules.Placement(piece.CellCount) + ScoreRules.Lines(clear.Lines, Combo);
            Tray.RemoveAt(trayIndex); Tray.Add(generator.Next());
            IsGameOver = !Board.HasAnyValidPlacement(Tray); return true;
        }

        public void Restart(int seed) { Board.Reset(); generator = new PieceGenerator(seed); Tray.Clear(); Score = 0; Combo = 0; PiecesPlaced = 0; LinesCleared = 0; IsGameOver = false; RefillTray(); }
